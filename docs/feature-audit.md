...
| Circuit Relay v2 fallback | Partial | Relay builds; 109 relay tests; NAT specs | Relay failover while an edit is in flight is not a default gate. |
...
| BeeKEM group key agreement | Partial | Primitive BeeKEM tree and Welcome suites | Active PathUpdate v1 cannot encode resolution fanout or safely apply every multi-level intersection; large-group churn and out-of-order delivery need convergence gates. |
...
| Member revocation and path updates | Partial | Primitive revocation, bounded wire/runtime admission, malformed-input, key-pair, resolution, atomicity, and FIFO suites | The active PathUpdate v1 protocol has no generation or parent-tree binding. Shipped keychains can incidentally reject an exact repeat of an already-installed epoch ID, but a previously unseen delayed/out-of-order update can roll state back. V1 fails closed when a blank sibling expands to multiple resolution nodes or a receiver's first intersection is below the root because one ciphertext per level cannot safely cover those cases. V2 is reserved but not wired into the runtime. Prove post-removal confidentiality and convergence in a hostile multi-peer test. |
...
| Relay server | Verified | TypeScript build, 109 tests, production container build, and a local two-restart identity/readiness smoke | Hosted restart, public WSS reservation, failover, capacity, and upgrade/rollback remain unverified. |
