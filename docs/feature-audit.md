...
| Member revocation and path updates | Partial | Primitive revocation, bounded wire/runtime admission, malformed-input, key-pair, atomicity, and FIFO suites | The active PathUpdate v1 protocol has no generation or parent-tree binding. Shipped keychains can incidentally reject an exact repeat of an already-installed epoch ID, but a previously unseen delayed/out-of-order update can roll state back. V1 also fails closed for non-root first intersections because one ciphertext per level cannot safely distribute the remaining ancestor keys. V2 is reserved but not wired into the runtime. Prove post-removal confidentiality and convergence in a hostile multi-peer test. |
