...
Control records use a strict versioned envelope. The MLS-specific control
payload uses deterministic DAG-CBOR and strict resource bounds. Unknown
...
version-1 MLS payload contains at least:
...
from remapping one operation to another same-member package. The store can
atomically move a pending package to a bounded irreversible consumed reference
while committing joined group state, so the same reference cannot be made
pending again by eviction.
