...
The charts below are generated directly from the benchmark suites on Node.js 22.

![CRDT Sync Pipeline Latency](/benchmarks/crdt-sync-latency.svg)

![Crypto Overhead: Plaintext vs Encrypted](/benchmarks/crypto-overhead.svg)

![Convergence Time by Peer Count](/benchmarks/convergence.svg)

![Index Query Scaling by Document Count](/benchmarks/index-query-scaling.svg)

![Bloom Filter Operations by Filter Size](/benchmarks/bloom-filter.svg)

![Blind Index Operations by Field Count](/benchmarks/blind-index.svg)

Charts are regenerated by running `node site/scripts/generate-benchmark-charts.mjs` after benchmark runs. The chart SVGs are committed to the repository alongside the benchmark data they visualize.
...
# Regenerate benchmark charts for the docs site
node site/scripts/generate-benchmark-charts.mjs
```

All suites output Markdown tables with statistical summaries and write `results.json`. The `--iterations` flag controls the sample count (default 100). The chart script reads `results.json` from both packages and generates SVG charts under `site/public/benchmarks/`.
