...
That metadata, including its revoked-key set, is neither replicated nor
enforced as shared revocation state; only the wrapped ACL's ordinary membership
change can reach other replicas.
