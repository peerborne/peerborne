...
- **Pinning is incomplete.** `PeerborneNode` does not subscribe to the legacy document-publish topic because V1 cannot authenticate or authorize pin requests, and the normal core commit path has no publisher. No generic IPFS pinning client exists. See [pinning cookbook](../../cookbook/pinning/).
