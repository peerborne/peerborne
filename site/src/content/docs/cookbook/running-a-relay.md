...
- `TOPIC_ALLOWLIST` defaults to `/peerborne/document/v3/` and limits which non-system GossipSub topics the relay node **auto-subscribes** to. Entries ending in `/` are namespace prefixes; entries without a trailing slash are exact topics, so `/announcements` does not admit `/announcements-extra`. Earlier default topics are rejected. All peers sharing a custom topic must use the same current runtime. Add every custom document prefix to each relay's allowlist. Configure permanent subscriptions with `EXTRA_TOPICS`. Set exactly `*` only for intentional open mode. Topic names are routing labels, not authenticated version negotiation. The allowlist does not inspect or restrict opaque, Noise-encrypted Circuit Relay streams and is not authentication, document authorization, or a confidentiality boundary.
