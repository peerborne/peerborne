...
- `DOCUMENT_PUBLISH_PATH` defaults to the v3 publish-notification topic `/peerborne/documents/v3`.
- `TOPIC_ALLOWLIST` defaults to `/peerborne/document/v3/,/peerborne/documents/v3,/document/,/documents` and limits which non-system GossipSub topics the relay node **auto-subscribes** to. Entries ending in `/` are namespace prefixes; entries without a trailing slash are exact topics, so `/peerborne/documents/v3` does not admit `/peerborne/documents/v30`. The v3 and legacy document and publish-notification topics remain separate migration namespaces; the relay does not mirror or bridge them. Mixed runtime versions on one custom topic are unsupported and must be upgraded together. Add every custom document prefix to each relay's allowlist; a custom publish-notification topic can instead be configured as `DOCUMENT_PUBLISH_PATH` or listed in `EXTRA_TOPICS`. Set exactly `*` only for intentional open mode. Topic names are routing labels, not authenticated version negotiation. The allowlist does not inspect or restrict opaque, Noise-encrypted Circuit Relay streams and is not authentication, document authorization, or a confidentiality boundary.
