...
- `TOPIC_ALLOWLIST` defaults to `/peerborne/document/v3/,/document/,/documents` and limits which non-system GossipSub topics the relay node **auto-subscribes** to. The v3 and legacy document prefixes remain separate migration namespaces; the relay does not mirror or bridge them. Mixed runtime versions on one custom prefix are unsupported and must be upgraded together. Set exactly `*` only for intentional open mode. The allowlist does not inspect or restrict opaque, Noise-encrypted Circuit Relay streams and is not authentication, document authorization, or a confidentiality boundary.
