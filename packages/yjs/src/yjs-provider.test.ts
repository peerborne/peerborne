...
describe('YjsACL', () => {
  ...
  test.each(['missing structs', 'pending deletes'] as const)(
    'prepareAdd() rejects ACL history with %s',
...
describe('YjsKeychain', () => {
  ...
  test('a failed merge hydration can be claimed without keys or retried', async () => {
    ...
    try {
      const unhydratedReceiver = new YjsKeychain();
      ...
      const retriedReceiver = new YjsKeychain();
...
describe('YjsJSONSerializer', () => {
  ...
  test('deserializeSyncMessage rejects non-object pathUpdate', () => {
    ...
    const wire = new TextEncoder().encode(
      JSON.stringify({ signatureContext: 'beekem-path-update-v1' as const, documentId: 'doc', pathUpdate: 'oops' }),
    );
  ...
  test('deserializeSyncMessage rejects null pathUpdate', () => {
    const serializer = new YjsJSONSerializer();
    const wire = new TextEncoder().encode(
      JSON.stringify({ signatureContext: 'beekem-path-update-v1' as const, documentId: 'doc', pathUpdate: null }),
    );
    expect(() => serializer.deserializeSyncMessage(wire)).toThrow(/pathUpdate/);
  });
