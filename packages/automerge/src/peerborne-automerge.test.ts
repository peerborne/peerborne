...
describe('AutomergeKeychain', () => {
  ...
  test('projection actor identity binds the complete key tuple', async () => {
    ...
    expect(decodeAutomergeChange(firstProjection[1]).actor).not.toBe(
      decodeAutomergeChange(secondProjection[1]).actor,
    );
...
describe('AutomergeJSONSerializer', () => {
  ...
  test('deserializeSyncMessage rejects pathUpdate that is not an object', () => {
    const wire = buildWire({ signatureContext: 'beekem-path-update-v2' as const,
      documentId: 'doc',
