...
describe('group-security store snapshot commitment', () => {
  test('preserves the exact canonical outbox encoding', () => {
    ...
    expect(toHex(canonicalGroupSecurityStoreSnapshot(value))).toBe(
      '70656572626f726e652f67726f75702d73656375726974792d73746f72652d' +
        ...
        '001c' +
        '67726f75702d73656375726974792e7472616e736974696f6e2e7631' +
        '0000000000000002' +
        '0000000000000003' +
        '00000002' +
        'abcd' +
  ...
  test('reports missing and non-callable Web Crypto digests consistently', async () => {
    ...
    for (const unavailable of [
      ...
      await withGlobalCrypto(unavailable, async () => {
        await expect(
          groupSecurityStoreSnapshotCommitment(value),
        ).rejects.toThrow('Web Crypto SHA-256 digest is unavailable');
      });
    }
  });

  test('invokes Web Crypto digest with the SubtleCrypto receiver', async () => {
    const value = await fixture();
    const originalCrypto = globalThis.crypto;
    const expected = await groupSecurityStoreSnapshotCommitment(value);
    const originalDigest = originalCrypto.subtle.digest;
    ...
    const subtle = {
      digest(
        ...
        receiver = this;
        return Reflect.apply(originalDigest, originalCrypto.subtle, [
          algorithm,
          data,
    ...
    await withGlobalCrypto({ subtle }, async () => {
      await expect(
        groupSecurityStoreSnapshotCommitment(value),
...
async function withGlobalCrypto(
  value: unknown,
  action: () => Promise<void>,
): Promise<void> {
