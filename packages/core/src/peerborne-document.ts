...
> {
  ...
  private async _makeChange(
    ...
    try {
      ...
      // Send new message.
      const updateMessage = this._createSyncMessage('ordinary-sync-v1');
  ...
  private async _loadInvitationCatchUp(
    ...
    // Sign inside each stream deadline so a provider that never settles
  ...
  private async _syncUnlocked(
    ...
    if (signingEnabled && verifySignature) {
      ...
      if (
        (await awaitLoadWork(
          this._verifyWriterSignature(unsigned.raw, signature!),
  ...
  public async acceptInvitationBootstrap(
    ...
    await this._runInvitationBootstrapStateApplication(
      async (beginStateApplication, bootstrapContinuation) => {
        ...
        let signatureBytes: Uint8Array;
        try {
          signatureBytes = this._deserializeSignature(
            bootstrapMessage.signature,
          );
        } catch {
        ...
        if (
          !(await syncInvitationMessageCompletely(
            ...
            {
              isSnapshotApplied: () =>
                this._isLatestSnapshotFrom(bootstrapMessageForSync),
            },
          ))
        ) {
  ...
  private async _evaluateAndApplyBeeKEMWelcome(
    ...
    if (this._writerKeysVersion !== writerKeysVersion) return 'retry';
