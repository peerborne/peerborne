...
describe('BeeKEM transactional state changes', () => {
  ...
  test('single-member roots do not depend on key extractability', async () => {
    const keys = await keyPair();
    const encoded = await crypto.subtle.exportKey('pkcs8', keys.privateKey);
    const protectedPrivateKey = await crypto.subtle.importKey(
      'pkcs8',
      encoded,
      ECDH_ALGO,
      false,
    ...
    const extractable = new BeeKEM();
    const protectedTree = new BeeKEM();
    await extractable.initialize(keys.privateKey, keys.publicKey);
    await protectedTree.initialize(protectedPrivateKey, keys.publicKey);
    expect(
      Buffer.from(await extractable.getRootSecret()).equals(
        Buffer.from(await protectedTree.getRootSecret()),
      ),
    ).toBe(true);
  });
  ...
  test('processWelcome validates a non-extractable recipient key without exporting it', async () => {
    ...
    await expect(
      bob.processWelcome(welcome, bobKeys.privateKey, wrongKeys.publicKey),
  ...
  test.each(['epoch mismatch', 'keychain failure'])(
    'transactional PathUpdateV2 commit rolls back on %s',
    async (failure) => {
      const alice = new BeeKEM();
      const aliceKeys = await keyPair();
      await alice.initialize(aliceKeys.privateKey, aliceKeys.publicKey);
  ...
  test('processPathUpdate applies and fingerprints one snapshot across an awaited digest', async () => {
    ...
    await expect(carol.processPathUpdate(forkA.pathUpdate)).resolves.toEqual(
      forkA.rootSecret,
    );
  ...
  test('runtime PathUpdateV2 snapshot rejects accessors, byte-view proxies, and shared backing', async () => {
    const alice = new BeeKEM();
    const aliceKeys = await keyPair();
    await alice.initialize(aliceKeys.privateKey, aliceKeys.publicKey);
    const bobKeys = await keyPair();
  ...
  test('v2 PathUpdateV2 cannot jump the append-only leaf topology', async () => {
    const alice = new BeeKEM();
    const aliceKeys = await keyPair();
    await alice.initialize(aliceKeys.privateKey, aliceKeys.publicKey);
    const bobKeys = await keyPair();
  ...
  test('v2 PathUpdateV2 enforces the exact leaf-level add, remove, and update rules', async () => {
    const alice = new BeeKEM();
    const aliceKeys = await keyPair();
    await alice.initialize(aliceKeys.privateKey, aliceKeys.publicKey);
    const bobKeys = await keyPair();
  ...
  test('concurrent updates serialize into distinct generations', async () => {
    ...
    const [first, second] = await Promise.all([alice.update(), alice.update()]);
