...
describe('BeeKEM multi-member churn', () => {
  ...
  test('rejects a v2 Welcome that omits a non-blank direct-path key', async () => {
    ...
    await expect(
      dave.processWelcome(malformed, daveKeys.privateKey, daveKeys.publicKey),
    ).rejects.toThrow(/omitted direct-path nodes must be blank/);
    expect(dave.memberCount).toBe(0);
    expect(dave.generation).toBeNull();
  });

  test('rejects a v2 PathUpdateV2 containing duplicate live leaf public keys', async () => {
    ...
    await bob.processWelcome(bobWelcome, bobKeys.privateKey, bobKeys.publicKey);
    const carolKeys = await keyPair();
    const { pathUpdate: addCarol } = await alice.addMember(carolKeys.publicKey);
    await bob.processPathUpdate(addCarol);
    ...
    await expect(bob.processPathUpdate(duplicateLeafUpdate)).rejects.toThrow(
      /duplicate live leaf public keys/,
    );
  ...
  test('rejects skipped generations instead of accepting a self-contained snapshot', async () => {
    ...
    const carolKeys = await keyPair();
    const { welcome: carolWelcome } = await alice.addMember(
      carolKeys.publicKey,
    );
  ...
  test('rejects a higher-generation stale fork that would re-include a revoked member', async () => {
    ...
    await expect(
      revoked.processPathUpdate(authoritativeRemoval),
  ...
  test.each([3, 4, 5])(
    ...
    async (memberCount) => {
      ...
      await expect(
        removed.beekem.processPathUpdate(pathUpdate),
      ).rejects.toThrow(/removed/);
    },
  );
